...
async function addVote(eventModel: Model<Event>, userModel: Model<User>, id: string, vote: VoteDto): Promise<Event> {
...
async function removeVote(eventModel: Model<Event>, userModel: Model<User>, id: string, vote: VoteDto): Promise<Event> {
  await validateData(eventModel, userModel, id, vote);
  const event = await eventModel.findById(id);
  const option = event.poll.options.find((opt) => opt.option === vote.option);
  const user = await userModel.findById(vote.userId);
...
async function getEventPollResults(eventModel: Model<Event>, id: string): Promise<PollOption[]> {
  const event = await eventModel.findById(id);
  if (!event) {
...
async function getOptionThatUserVotedFor(eventModel: Model<Event>, userModel: Model<User>, id: string, userId: string): Promise<number> {
  const event = await eventModel.findById(id);
  if (!event) {
    throw new NotFoundException(`Event with id ${id} not found`);
  }
