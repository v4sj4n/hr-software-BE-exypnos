...
export class SalaryService {
  ...
  async create(
    ...
    for(const salary of salarys) {
      if (this.ismatch(createSalaryDto.userId as unknown as string, createSalaryDto.month, createSalaryDto.year, salary.uniqueId)) {
        throw new ConflictException('Salary already exists for this user in this month');
      }
    }
  ...
  private async createUniqueId( salary: CreateSalaryDto | UpdateSalaryDto): Promise<string> {
  ...
  private async ismatch(userId: string, month: number, year: number, uniqueId: string): Promise<boolean> {
...
//for later use
  ...
  async getSalary(
    ...
    if (!(await this.ismatch(userId, month, year, uniqueId))) {
      throw new NotFoundException(`Salary with user id ${userId} in ${month},${year} not found`);
    }
