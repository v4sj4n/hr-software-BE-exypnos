...
@WebSocketGateway({
  cors: {
    origin: process.env.FRONT_URL, // Replace with your frontend URL if it doesnt work
  },
})
  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleMessage(
    ...
    if (recipientSocketIds) {
      recipientSocketIds.forEach((socketId) => {
        console.log(
          `Sending message to ${payload.recipientId} at socket ${socketId}`,
        );
  ...
  handleDisconnect(client: Socket) {
    const userId = [...this.activeUsers.entries()].find(([, socketIds]) =>
    ...
    if (userId) {
      ...
      this.logger.warn(
        `Disconnection of socket ${client.id} not linked to any user`,
      );
    }
  }
